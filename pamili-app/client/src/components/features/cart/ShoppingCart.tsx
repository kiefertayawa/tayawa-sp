import { X, Plus, Minus, Trash2, ShoppingBag, HelpCircle } from 'lucide-react';
  ...
  return (
    <>
      ...
      <div className="fixed top-0 right-0 h-full bg-white z-50 flex flex-col shadow-2xl" style={{width:'460px'}}>
                ...
                return (
                  <div key={storeName} className="border border-gray-100 rounded-xl overflow-hidden">
                    <div className="px-4 py-3 bg-white border-b border-gray-50">
                      <p className="font-semibold text-sm" style={{color:'#8B1538'}}>{storeName}</p>
                    </div>
                    <div className="divide-y divide-gray-50">
                      {storeItems.map(item => (
                        <div key={`${item.productId}-${item.storeId}`} className="flex gap-3 px-4 py-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2">
                              <button onClick={() => updateQuantity(item.productId, item.storeId, item.quantity - 1)} className="w-6 h-6 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50">
                          ...
                          <div className="flex flex-col items-end justify-between">
                            <button onClick={() => removeItem(item.productId, item.storeId)} className="p-1 text-gray-300 hover:text-red-400">
        ...
        {/* Help button */}
        <button className="absolute bottom-4 right-4 w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center">
